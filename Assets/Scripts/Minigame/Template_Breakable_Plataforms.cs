...
public class Template_Breakable_Plataforms : MonoBehaviour
{   
    Transform plat_t;
    float timer = 1f;
    float timer2 = 0.5f;
    ...
    void Update()
    {
        ...
        Collider2D[] hit = Physics2D.OverlapBoxAll(plat_t.position, new Vector2(2f,05f),0f);
    ...
    {
        if(plat_t != null){
            Gizmos.DrawWireCube(plat_t.position,new Vector3(2f,0.5f,0f));
        }
    }
}
